...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // First, find or create the Stripe product for this plan
    const productName = `Healthspan ${plan.label}`;
    let product;
    const existingProducts = await stripe.products.search({ query: `name:"${productName}"`, limit: 1 });
    if (existingProducts.data.length > 0) {
      product = existingProducts.data[0];
    } else {
      product = await stripe.products.create({ name: productName, metadata: { plan_type: planType } });
    }
